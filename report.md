...
Goal 5 现获三条独立且互补的证据线，每条覆盖一个独立维度：访问控制（[10]）、跨轨迹自演进（[16]）、持久度治理（[17]）。

| 维度 | [10] Collaborative Memory | [16] G-Memory | [17] FadeMem |
|------|--------------------------|----------------|---------------|
| 核心问题 | 谁能看到什么 | 如何从历史学习 | 什么应该被保留多久 |
| 时间范围 | 跨 session（用户级） | 跨 trial（任务实例级） | 跨 session 持续衰减（fragment 级） |
| 主体 | 人类用户 × agent | Agent × agent | Fragment × 时间 |
| 授权模型 | 正式化二部图 ACL | 无授权模型 | 无授权模型 |
| 内容类型 | 用户事实、偏好片段 | MAS 协作轨迹洞见 | 任意 fragment + 重要性强度 |
| 写入机制 | 用户/agent 显式写入 | LLM 自动生成 | LLM 仲裁冲突 + 融合 |
| 治理风险 | 低 | 高（黑盒 insight） | 中（冲突分类 acc 66.2%）|
| TraceAI 友好度 | 高（显式 provenance） | 低（LLM 黑盒生成） | 高（衰减闭式可前向还原） |

**堆叠设计**：在同一系统中，[10] 处理"这个用户的记忆是私有还是共享、谁的 agent 能访问"；[16] 处理"过往任务里 Analyst agent 与 Reviewer agent 的协作 insight 应在第几步注入"；[17] 处理"用户三个月前的偏好片段是否仍应作为关键事实参与召回、与新偏好冲突时如何抑制"。三层职责不重叠，但写入路径需协调：[17] 的衰减状态须按 (fragment, user) 元组独立维护，不能让 A 用户的高频访问减缓 B 用户共享空间的同条 fragment 衰减——这是 [17] 单用户假设留下的、必须在合成时补齐的关键约束。
...
[10]+[16]+[17] 三层堆叠后，v14 列出的三个文献空白部分填补，但合成路径上的工程债务转移到三处新边界：

1. **多租户独立衰减**：[17] 单用户假设让"跨租户 fragment 衰减是否独立维护"完全空白——必须把 [10] 的 (T, U, A, R) provenance 扩展为 (T, U, A, R, v, λ, β, layer, last_access, prune_reason)，且衰减状态按 (fragment, user) 元组而非 fragment 全局维护。
2. **LLM 仲裁的合规安全**：[17] 冲突分类 acc 66.2% / [10] redact 转写无失败率评测——两者共享同一安全弱点，企业合规场景必须"BKN schema 规则前置 + LLM 兜底"分轨，不能让 LLM 概率输出独立决定权威源。
3. **写入路径吞吐**：[17] 同步 LLM 写入与 thesis "7×24 数字员工"目标存在工程张力，需异步化（先入库 + 后台合并）+ BKN 规则前置过滤的工程评测。

这三处是 goal 5 v0.8 → v1.0 的剩余工程债务，但已从"机制空白"降级为"工程参数化与多租户扩展"——证据完整度显著提升。
