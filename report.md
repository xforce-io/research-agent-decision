...
> **Version:** v3.1 (7 papers)
> **Last Updated:** 2026-05-01
> **Papers:** [01](notes/01_co_evolving_llm_decision_and_skill.md) COS-PLAY · [02](notes/02_graph_of_agents_a_graph_based.md) GoA · [03](notes/03_why_reasoning_fails_to_plan_a.md) FLARE · [04](notes/04_rethinking_the_value_of_multi_agent.md) OneFlow · [05](notes/05_agent_as_a_graph_knowledge_graph.md) Agent-as-a-Graph · [06](notes/06_why_do_multi_agent_llm_systems.md) MAST · [07](notes/07_mcp_zero_active_tool_discovery_for.md) MCP-Zero
> **Thesis:** [.researcher/thesis.md](.researcher/thesis.md)

> **形式：** 内部技术报告 / Position Paper
> **作者：** xupeng
> **基于：**
> - 7 篇深读论文（COS-PLAY、GoA、FLARE、OneFlow、Agent-as-a-Graph、MAST、MCP-Zero）— 索引见 [`notes/`](notes/)
...
> **每次 `researcher run` 累加的论文都会修正或证伪 thesis 的某个判断。本节把 7 轮校准的"已被证伪 / 修正 / 支持"汇总——避免历史噪音被新读者误读。**

| 原始判断（thesis v0 / 项目资料） | 校准后（v1–v7 之后） | 触发论文 |
|---|---|---|
...
| "BKN 语义解耦在超大工具集场景下的有效性尚缺乏系统性验证" | **两条独立证据线均支持但规模未触及**：(a) [5] 类型化节点 + 加权 RRF 相对"拼接单向量"基线 Recall@5 +14.9%（LiveMCPBench 527 tools）；(b) [7] "LLM 主笔结构化请求"相对"用户查询直接检索"基线在 APIBank 多轮 Full 集（48 tools）上 +25 个百分点（90.32% vs 65.05%），token −97.52%。两条路径在 LiveMCPBench 直接对照中 [5] 胜出（Recall@5 0.85 vs 0.70）。规模缺口未变：均远低于企业目标 5k–10k | [5] Agent-as-a-Graph、[7] MCP-Zero |
...
| "强模型也需要主动检索来抑制工具幻觉" | **被部分证伪**：[7] 显示 GPT-4.1 在 needle-in-haystack（1–2,797 tools）上对 MCP-Zero 无精度增益（baseline 已饱和），仅 token 节省。"主动检索"对精度的杠杆主要在弱模型；Decision Agent 若以最强基础模型落地，活跃检索的实际增益更可能落在 token 成本侧而非精度侧 | [7] MCP-Zero |
...
| 3 高质量上下文（精准装配 + 按需加载）| **Agent-as-a-Graph [5] / MCP-Zero [7]**：类型化召回与 BKN "逻辑/行动分离"同构（[5]）；模型主笔结构化请求把"用户查询→工具描述"扩为"server+tool 两段式"，APIBank −97.52% token（[7]）|
| 4 多智能体协同（统一语义 + 动态调度）| **OneFlow [4] / GoA [2] / MAST FC2 [6]**：异质性审计 + 调度参考 + 失败诊断 |
| 5 安全控制（可审计防御围栏）| 无直接论文；可观测性范式可借 MAST IAA 做基础 |

### 1.3 五大业务价值的学术不可证伪性

references/01 §五大业务价值（决策提效 / 知识可信 / 流程自动化 / 数据资产可用 / 风险可控）是产品价值主张，**不在 7 篇论文的可证伪范围内**——它们由企业落地数据（不是公开 benchmark）证明或证伪。研究输入只能服务于"实现这 5 个价值的工程机制选择"，无法直接背书价值主张本身。

---

## 2. 当前差距：从 references/01 痛点到学术映射

> references/01 §当前核心痛点列出 5 个待研究支撑的设计缺口。本节把每个痛点对照到 7 篇论文，明确"哪些已被覆盖、哪些仍是开放空白"。
...
| 4 | **工具幻觉**：模型越强越严重，BKN 语义解耦待验证 | 中-高 | Agent-as-a-Graph [5] 类型化节点 +14.9% Recall@5（与 BKN 同构）；MCP-Zero [7] "模型主笔请求 vs 用户查询直接检索" 在 APIBank Full（48 tools）+25 个百分点；两路径在 LiveMCPBench 直接对照中 [5] 胜出（0.85 vs 0.70）| 527/48 工具远低于企业 5k–10k；KWeaver 自家工具集复测未做；αA:αT 自家最优值未知；[7] 在强模型上无精度增益，BKN 数据上是否复现待测；"LLM 写、LLM 检"循环对评测有效性的影响未控制 |
...
> 不是论文综述，而是把 7 篇论文重组到上面 5 个痛点 + 1 个先决问题。每节末尾给出对 Decision Agent 的可操作启示。
...
### 3.4 痛点 4（工具幻觉）→ Agent-as-a-Graph + MCP-Zero 双路径实证

**两条独立证据线，方向不同但同向支持 BKN 思想。**

[5] Agent-as-a-Graph 在 LiveMCPBench（527 工具）上把工具/agent 建为二部图两类节点 + 加权 RRF 检索：Recall@5 = 0.85 vs. ScaleMCP 的 0.74（+14.9%）。**这 14.9% 不是更好的 embedding，而是把"逻辑（agent 描述）"和"行动（工具描述）"从同一向量拆出来——结构上与 references/02 §方案二（BKN 驱动语义精准执行）完全同构**。最优类型权重 αA:αT = 1.5:1（轻度偏向 agent 覆盖）；架构无关（跨 8 种 embedding 模型 std≈0.02）。

[7] MCP-Zero 从相反方向给出第二份证据：让 LLM 主动生成 `<tool_assistant>{server, tool}` 结构化请求触发外部检索，替代"全部 schema 注入 prompt"。APIBank 多轮 Full（48 unique tools，Claude-3.5-Sonnet）：标准 schema 注入精度从 Domain 子集 100% 跌至 Full 60.22%，MCP-Zero 维持 90.32%；token 消耗 6402.2→159.0（−97.52%）。**关键归因**：仅做"用户查询→工具描述"的 Q.Retrieval 基线只有 65.05%，落后 MCP-Zero 25 个百分点——证明"由模型主笔语义对齐请求"是关键收益来源，与 BKN "逻辑/行动分离"思想同向（请求按 server/tool 两段分笔，本质是把"做什么"和"在哪做"解耦）。

**两路径直接对照（LiveMCPBench, 527 tools）**：[5] Recall@5 = 0.85 系统性击败 [7] 的 0.70 [5: §4.2, Table 1]——静态目录上的"显式类型节点 + 类型加权 RRF"在该规模下优于"运行时 LLM 主笔请求 + 双层余弦"。两者可工程化组合：MCP-Zero 的请求生成 → Agent-as-a-Graph 的类型化检索后端。

**强模型上的边界条件 [7: Figure 5]**：MCP-Zero 在 GPT-4.1 上对 needle-in-haystack 精度无增益（baseline 已饱和），仅 token 节省。Decision Agent 若以最强基础模型落地，活跃检索的实际增益更可能落在**token 成本侧**而非精度侧——需修正"主动检索能提升精度"的潜在过度承诺。

**对 Decision Agent 的可操作启示**：
- **P0**：KWeaver 自家工具集（构建 5k+ 规模）复测类型化结构 vs. 拼接基线（详见 §4 P0-3）；同时在自家轨迹上对比"模型主笔请求 vs 用户查询直接检索"作为 ContextLoader 前置请求改写器的可行性
- **P1**：暴露 αA:αT 作为 ContextLoader 的可调旋钮——查询意图模糊时偏向父技能/父域覆盖（αA 高），意图明确时偏向工具级精准（αT 高）
- **缺口**：(a) [5] 不报 wall-clock 延迟也不分析图遍历成本，5k–10k 工具下是否仍可接受需自家测；(b) [7] APIBank Full 仅 48 工具，多步、跨域链式检索精度未验证；(c) [7] 的 "LLM 写、LLM 检"循环（summary 由 Qwen2.5-72B 生成，请求由 Claude/GPT/Gemini 生成）可能放大同空间伪信号，BKN 数据上是否复现待测
...
### P2 ContextLoader 类型权重旋钮 + 主动请求改写器 + 调度元层
**动机**：[5] 暴露 αA:αT 作为可调；[7] 暴露"模型主笔请求"作为查询前置改写的 25 个百分点抓手；[2] GoA 提供 openclaw → decision agent 的元调度参考。
**操作**：先做 P0-3 复测确认 [5] 数字可迁移，再把 αA:αT 暴露为 BKN 检索 API 参数；并实验"用户查询 → LLM 改写为结构化请求 → BKN 检索"链路替代直接查询；调度元层作为 openclaw 工程化时的备选实现。

---

## 5. 阻塞与限制

> 7 篇论文都验证不了的事情，需要 Decision Agent 团队自身回答。
...
3. **企业工具集 5k–10k 规模的检索延迟**：Agent-as-a-Graph [5] 在 527 工具量级验证、MCP-Zero [7] APIBank 仅 48 unique tools；两者均不报 wall-clock 或缩放曲线，ContextLoader 真正起作用的量级需自测。
...
7. **"LLM 写、LLM 检"循环对工具检索 benchmark 的污染**：MCP-Zero [7] 的 server summary 由 Qwen2.5-72B 生成，请求由 Claude/GPT/Gemini 生成，目标工具描述也是 LLM 重写——三处均处 LLM 语义空间，可能放大同空间伪信号。Decision Agent 的 BKN 工具描述若来自人工产品同事撰写，[7] 的精度数字未必可迁移；KWeaver 自家 P0-3 复测必须用真实工具文档而非 LLM 重写版本。
...
| ContextLoader（动态加载）| dynamic / lazy tool retrieval / active tool discovery | [5] sources distribution: 39.13% agent + 34.44% tool via owner edge；[7] `<tool_assistant>{server, tool}` 主动请求块 |
| ContextLoader 前置改写器 | model-authored structured request | MCP-Zero [7] 把"用户查询"改写为"server + tool"两段式请求 |
...
| 工具幻觉 | tool hallucination | references/02 §1；[5] 缓解 via 类型化召回；[7] 缓解 via 主动请求 + 双层向量路由 |
...
| 6 | 让模型主笔结构化请求比直接用查询检索精度更高 | KWeaver 自家轨迹复测"模型主笔请求 vs 用户查询直接检索"；若强模型上 ΔRecall<5% 则只剩 token 节省价值 | MCP-Zero [7] |
| 7 | 主动检索能在强模型上提升精度 | [7] Figure 5 已部分证伪（GPT-4.1 baseline 饱和无增益）；KWeaver 用最强基础模型时需独立验证 | MCP-Zero [7] |
...
- [07] MCP-Zero: Active Tool Discovery for Autonomous LLM Agents (Fei et al., 2025)
...
| 版本 | 日期 | 新增论文 | 关键变化 |
|---|---|---|---|
| v1 | 2026-04-27 | [01] COS-PLAY | 建立技能库协同进化基线 |
| v2 | 2026-04-27 | [02] GoA | 推理时图路由参考 |
| v3 | 2026-04-27 | [03] FLARE | 步进式推理结构性局限诊断 |
| v4 | 2026-04-27 | [04] OneFlow | 多智能体必要性挑战 |
| v5 | 2026-04-29 | [05] Agent-as-a-Graph | BKN 语义解耦首份学术实证 |
| v5.1 | 2026-04-30 | [06] MAST | 横切诊断维度（PR #9）|
...
| v3.1 | 2026-05-01 | [07] MCP-Zero | §0 校准表合并 [5]+[7] 工具检索两条独立证据线；§3.4 改为"双路径实证"，加入 LiveMCPBench 直接对照（[5] 0.85 vs [7] 0.70）与 GPT-4.1 强模型边界条件；§5 新增"LLM 写、LLM 检"循环污染阻塞；附录 B 新增 #6/#7 可证伪点；附录 A 新增 ContextLoader 前置改写器条目 |
