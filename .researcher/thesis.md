...
**Goal 3 (Shared Workspace) 与 goal 4 (Composer) 同样需要治理分轨设计**（由 [9] Blackboard MAS 阅读触发，notes/09）。[9] 的 β/β_r 双板设计（共享黑板 β 接受请求广播，私有响应板 β_r 隔离 helper 响应防 cross-influence）在 master-slave 基线上获 13–57% 相对提升，提供 goal 3 候选 default 设计；helper 自荐机制（无中心能力注册表）也为 goal 4 Composer "语义请求 + 候选 Agent 自荐" 提供初步支持。但三个边界条件必须显式纳入设计：(a) **β_r 私有 vs TraceAI 全审计的张力**——helper 响应被隔离在主智能体独占空间，跨 agent trace 视图缺失；Decision Agent 落地决议是按动作类型分轨：内部信息处理（检索 / 汇总 / 推理中间步）走 β_r 私有 + TraceAI 镜像，涉及外部副作用（写库 / 调用工具 / 发邮件）必须直接进 β + Human-in-the-loop；(b) **"Helper autonomy" 是 prompt 修辞而非架构属性**——[9] 中 helper LLM 被 prompt 问"你能否处理"，本质是 LLM-as-router；不能因此放弃 BKN-anchored capability lookup，需对照"广播自荐 vs 结构化 BKN 查询"在 5k+ 工具/Agent 规模下的 Recall@5；(c) **规模差两个数量级**——[9] helper 上限 27，Decision Agent 目标 5k–10k；broadcast 成本线性增长，5k 下不可行，需保留 BKN 预过滤回退路径。**OneFlow [4] 的折叠等价被发现是有边界的**——隐含"任务 fits 单 LLM 上下文"前提，[9] 的 DS-GRU baseline（Qwen3 1.21% / Claude 4 Opus 4.11%）在数据湖文件总量超出上下文窗口时反证该等价物理上无法承载。Decision Agent 的"多 agent 必要性 vs 折叠等价"判断因此须按"上下文 fit / 不 fit"分子情形决策。
...
- `notes/09_llm_based_multi_agent_blackboard_system.md` — Blackboard β/β_r 双板设计为 goal 3/4 提供候选原型，但暴露规模 + 治理 + 自荐协议三处边界
...
| Shared Workspace 能显著降低 FC2 协同失真 | 若学术工作显示共享存储反而加重协调成本 | [9] Blackboard 提供间接工程证据（master-slave → broadcast/β_r 13–57% 相对提升）；但 devil's-advocate 指出 baseline 不公平 + 规模仅 27 helper，证据强度 [med] 而非 [high] |
| Shared Workspace 设计须按动作类型分轨：内部信息处理 vs 外部副作用 | 若发现"全部 helper 响应直接进 β + Human-in-the-loop"在治理事件率与吞吐两个维度都不输于"分轨"方案，则该决议失去依据 | 已决议（contradiction 1 → Option C 改良版）：内部处理走 β_r 私有 + TraceAI 镜像；涉及外部副作用必须直接进 β + Human-in-the-loop |
| OneFlow 折叠等价适用于 Decision Agent | 若发现 Decision Agent 真实场景（数据湖 / 多文档检索）任务规模超出单 LLM 上下文，则 [4] 等价构造物理上无法承载——已被 [9] DS-GRU baseline 证伪在该子情形 | 已决议（contradiction 2 → B+A）：landscape [4] 加边界注记；report §0 拆"上下文 fit / 不 fit"两子情形 |
